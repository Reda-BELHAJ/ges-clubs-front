...
export default class NavbarAuth extends Component{
    ...
    componentWillUnmount() {
        ...
        const scrollHandler = () => {
            window.pageYOffset > 10 ? this.setState({
                top: false
            }) : this.setState({
                top: true
            });
    ...
    render() {
        ...
        return (
            <header className={`fixed w-full z-30 md:bg-opacity-90 transition duration-300 ease-in-out ${!this.state.top && `bg-white backdrop-blur-sm shadow-lg`}`}>
